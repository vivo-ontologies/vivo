...
1. Add an import statement to your `src/ontology/vivo-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
    Ontology(<http://vivoweb.org/ontology/core/vivo.owl>
    Import(<http://vivoweb.org/ontology/core/vivo/imports/ro_import.owl>)
    Import(<http://vivoweb.org/ontology/core/vivo/imports/go_import.owl>)
    ...
    ```
2. Add your imports redirect to your catalog file `src/ontology/catalog-v001.xml`, for example:
    ```
    <uri name="http://purl.obolibrary.org/obo/vivo/imports/go_import.owl" uri="imports/go_import.owl"/>
    ```
...
```
Import(<http://purl.obolibrary.org/obo/vivo/imports/go_import.owl>)
```

in your editors file (the ontology) and

```
<uri name="http://vivoweb.org/ontology/core/vivo/imports/go_import.owl" uri="imports/go_import.owl"/>
```
